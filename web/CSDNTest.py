...
def main():
    ...
    titleList = []
    numList = []
    for indexHtml in range(1, 2):
        soup = NetUtil.getBeautifulSoup(
            "https://blog.csdn.net/coder_pig/article/list/%d" % indexHtml)
        # if soup is None:
        #     sys.exit()

        items = soup.find_all(name="div",
                              attrs={"class": "article-item-box csdn-tracking-statistics"})
